...
def _get_end_to_end_prefix_allowed_tokens_fn(
    ...
    # print(encode_fn("^"), encode_fn("^ "), encode_fn(" ^"), encode_fn("^ {"))
    sent_origs = [[codes["EOS"]] + [bos_token_id] + encode_fn(sent)[1:] for sent in sentences]
    # sent_origs = [[bos_token_id] + encode_fn(sent)[1:] for sent in sentences]
    # sent_origs = [encode_fn(sent) for sent in sentences]
    ...
    def get_trie_outside(sent, sent_orig):
        ...
        if pointer_end:
            if sent_orig[pointer_end] != codes["EOS"] and sent_orig[
                ...
                # print("HERE", [sent_orig[pointer_end]])
                return [sent_orig[pointer_end]]
        else:
            return [codes["EOS"]] # New version of transformer requires explicit EOS as last output
