...
class Trie(object):
    ...
    def get(self, prefix_sequence: List[int]):
        # Empty lists no longer allowed in prefix_allowed_tokens, see https://github.com/facebookresearch/GENRE/issues/103
        curr_options = Trie._get_from_trie(
            prefix_sequence, self.trie_dict, self.append_trie, self.bos_token_id
        )
        if not curr_options:
            curr_options = [2] # EOS token
